...
public class Put_Message_Dialog extends DialogFragment implements Observer
{
    ...
    public Dialog onCreateDialog( Bundle savedInstanceState )
    {
        ...
        builder.setMessage( "" ).setTitle( "Sending..." );

        thisView = getActivity().getLayoutInflater().inflate( R.layout.progress_bar_dialog, null );
        builder.setView( thisView ).setNegativeButton( "  .  ", new DialogInterface.OnClickListener()
        {
            @Override
    ...
    public void update( Observable observable, Object o )
    {
        ...
        {
            Log.i( "Post message Dialog", "UPDATE" );
            mHandler.post( () ->
                           {
                               ...
                               if( o instanceof ObserverMessage )
                               {
                                   ...
                                   {
                                       case Post_Mesaj_Completed:
                                           dismiss();
                                           break;
                                       case Post_Mesaj_Network_Error:
                                           ( ( TextView )( thisView.findViewById( R.id.dialog_txt ) ) ).setText( "A network error occured." );
                                           this_dialog.setButton( DialogInterface.BUTTON_NEGATIVE, "Close", ( DialogInterface dialog, int id ) ->
                                           {
                                               dismiss();
                                           } );
                                           break;
                                       case Post_Mesaj_Refused:
                                           ( ( TextView )( thisView.findViewById( R.id.dialog_txt ) ) ).setText( "Sending failed! The server refused the credentials" );
                                           this_dialog.setButton( DialogInterface.BUTTON_NEGATIVE, "Close", ( DialogInterface dialog, int id ) ->
                                           {
                                               dismiss();
                                           } );
