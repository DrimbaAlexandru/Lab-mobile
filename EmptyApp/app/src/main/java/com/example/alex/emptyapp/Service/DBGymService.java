...
/*
    ...
    public GymClass addClass( GymClass c ) throws IllegalArgumentException
    {
        boolean ok = true;
        String error = "";
        for( int tId : c.getClassTrainers() )
        {
            User u = userRepo.getById( tId );
            if( u == null )
            {
                ok = false;
                error += "User with id " + tId + " does not exist\n";
            }
            else
            {
                ...
                {
                    ok = false;
                    error += "User with id " + tId + " is not a trainer\n";
                }
            }
        }
        if( ok )
    ...
    public ClassSchedule addClassSchedule( ClassSchedule cs ) throws IllegalArgumentException
    {
        boolean ok = true;
        String error = "";
        for( int tId : cs.getClassParticipants() )
        {
            User u = userRepo.getById( tId );
            if( u == null )
            {
    ...
    public void updateClass( GymClass gymClass )
    {
        boolean ok = true;
        String error = "";
        for( int tId : gymClass.getClassTrainers() )
        {
            User u = userRepo.getById( tId );
            if( u == null )
            {
